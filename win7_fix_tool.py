...
def check_wia_service():
    """检查WIA扫描服务是否正常运行"""
    try:
        result = subprocess.run(['sc', 'query', 'stisvc'], capture_output=True, text=True, timeout=5)
        return 'RUNNING' in result.stdout
    except:
        return False
...
def main():
    """主程序入口"""
    print("=" * 60)
    print("Win7打印及扫描环境修复工具 2.0版  by 忆痕（yckj666@52PJ）")
    ...
    print(f"扫描仪驱动：{'✅ 已安装' if scanner_ok else '⚠️ 未检测到扫描设备'}")
