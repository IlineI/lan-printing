...
def get_available_scanners():
    ...
    scanners = []
    try:
        # 方法1: 使用WIA (Windows Image Acquisition) COM对象
        try:
            ...
            for i in range(device_infos.Count):
                device_info = device_infos.Item(i + 1)  # WIA索引从1开始
                
                # 检查是否为扫描仪设备
                device_type = device_info.Properties("Type").Value
                if device_type == 1:  # 1表示扫描仪
...
Get-PnpDevice | Where-Object {
    ...
    $_.FriendlyName -like "*扫描*" -or
    $_.HardwareID -like "*scan*"
...
'''
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                for device in devices:
                    device_name = device.get('FriendlyName', 'Unknown Scanner')
...
def start_scan_silent(scanner_id, scanner_name, scan_format='PNG', scan_path=None):
    ...
    try:
        ...
        # 方法1: 使用WIA COM对象进行静默扫描
        try:
            import comtypes.client
            
            print("尝试使用WIA COM对象进行静默扫描...")
            ...
            # 查找指定的扫描仪设备
            device = None
            if scanner_id != 'default':
                # 尝试通过ID查找设备
                device_infos = device_manager.DeviceInfos
                for i in range(device_infos.Count):
                    device_info = device_infos.Item(i + 1)
                    if device_info.Properties("DeviceID").Value == scanner_id:
            ...
            if device is None:
                ...
                if device_infos.Count > 0:
                    device_info = device_infos.Item(1)  # 使用第一个设备
                    device = device_info.Connect()
                else:
                    raise Exception("未找到可用的扫描仪设备")
            
            print(f"已连接到扫描设备: {device.Properties('Name').Value}")
            
            # 获取第一个扫描仪项目（通常是平板扫描）
            items = device.Items
            if items.Count == 0:
                raise Exception("扫描仪没有可用的扫描项目")
            ...
            try:
                # 设置扫描格式
                if scan_format.upper() == 'PDF':
                    format_guid = "{B96B3CAE-0728-11D3-9D7B-0000F81EF32E}"  # PDF格式
                elif scan_format.upper() == 'JPEG':
                    format_guid = "{B96B3CAE-0728-11D3-9D7B-0000F81EF32E}"  # JPEG格式
                else:  # PNG或其他
                    format_guid = "{B96B3CAF-0728-11D3-9D7B-0000F81EF32E}"  # PNG格式
                
                # 设置扫描分辨率 (DPI)
                scanner_item.Properties("6146").Value = 300  # 设置为300 DPI
                
                # 设置颜色模式 (1=黑白, 2=灰度, 4=彩色)
                scanner_item.Properties("6146").Value = 4  # 彩色扫描
                
                print(f"扫描参数设置完成: 格式={scan_format}, 分辨率=300 DPI, 模式=彩色")
                
            except Exception as e:
                print(f"设置扫描参数时出错: {e}, 使用默认设置")
            
            # 执行静默扫描
            print("正在执行静默扫描...")
            image_file = scanner_item.Transfer(format_guid)
            
            if image_file is None:
                raise Exception("扫描返回空结果")
            
            # 保存扫描结果
            image_file.SaveFile(full_scan_path)
            print(f"扫描完成，文件已保存: {full_scan_path}")
            ...
            if os.path.exists(full_scan_path) and os.path.getsize(full_scan_path) > 0:
                ...
                return False, "扫描文件保存失败"
            
        except Exception as e:
            print(f"WIA COM扫描失败: {e}")
            
            # 方法2: 使用Windows扫描命令行工具
            try:
                print("尝试使用Windows扫描命令行工具...")
                
                # 使用PowerShell + Add-Type调用Windows.Devices.Scanners
                ps_script = f'''
try {{
    Add-Type -AssemblyName System.Runtime.WindowsRuntime
    
    # 创建扫描任务
    $scanPath = "{full_scan_path.replace(chr(92), chr(92)+chr(92))}"
    
    # 使用WinRT扫描API（Windows 10+）
    [Windows.ApplicationModel.Core.CoreApplication, Windows.ApplicationModel, ContentType = WindowsRuntime] | Out-Null
    
    # 这里使用简化的方式：调用系统扫描程序
    Start-Process -FilePath "WiaAcmgr.exe" -ArgumentList "/SelectDevice" -WindowStyle Hidden -Wait
    
    Write-Output "扫描命令已执行"
}} catch {{
    Write-Output "扫描失败: $_"
    exit 1
}}
'''
                ...
                print(f"PowerShell扫描失败: {e}")
            
            # 所有静默扫描方法都失败了
            return False, "扫描失败: 未检测到可用的扫描仪或扫描仪不支持静默扫描\n\n可能原因：\n• 扫描仪未正确连接或开机\n• 缺少WIA驱动程序\n• 扫描仪不兼容静默扫描\n• 权限不足或安全软件阻止\n\n建议：如果确实安装了扫描仪，但是没检测出来且扫描失败，请在目标电脑上进行扫描操作"
...
<body>
<div class="container">
    <h1 class="mb-4 text-center">内网打印及扫描服务</br><span class="author-info">（yckj666@52PJ，作者：忆痕）【<a href="https://github.com/a937750307/lan-printing" target="_blank" style="color: #007bff; text-decoration: none;">Github</a>】</span></h1>
