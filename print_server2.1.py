...
class ServiceManager:
    ...
    def _optimize_for_windows_version(self):
        ...
        try:
            ...
            self.restart_count = 0
            self.last_restart_time = 0
            print(f"⚠️ Windows版本检测失败，使用默认服务管理设置: {e}")
...
def get_optimized_flask_config():
    ...
    try:
        ...
        print(f"⚠️ Flask配置检测失败，使用默认配置: {e}")
                ...
                <div class="card-body">
                    ...
                    <div class="alert alert-info mt-3">
                        ...
                        <ul class="mb-0 small">
                            ...
                            <li><strong>🖼️ 支持格式:</strong> PNG、JPEG、PDF格式，建议使用PNG获得最佳质量</li>
                            <li><strong>⚙️ 自动设置:</strong> 尝试自动设置300 DPI分辨率和彩色扫描模式</li>
                            <li><strong>📁 自动保存:</strong> 扫描完成的文件会保存在【扫描文件】区域，支持预览、下载和打印</li>
                            <li><strong>🔧 兼容性:</strong> 兼容大多数品牌的扫描仪，如Canon、HP、Epson等</li>
                            <li><strong>⚠️ 失败处理:</strong> 如果确实安装了扫描仪但扫描失败，请在目标电脑上进行扫描操作</li>
                        </ul>
                    </div>
                </div>
            </div>
    ...
    try:
        ...
        if hasattr(sys, '_MEIPASS'):
            ...
            if is_win7:
                ...
                print("💡 Win7模式：控制台保持可见，避免托盘兼容性问题")
                # Win7下不隐藏控制台，避免托盘问题
                ...
                print("�️ 检测到Windows 10系统，启用标准模式...")
...
如果问题持续，建议使用其他端口号（如5001-5010）"""
        ...
        if local_ip == '127.0.0.1':
            ...
            if len(PRINTERS) > 0:
                print(f"   📍 本机访问: http://127.0.0.1:{port}")
                print(f"   🖨️ 可用打印机: {len(PRINTERS)} 台")
                print("   ✅ 本地打印功能完全正常")
            else:
                print(f"   📍 本机访问: http://127.0.0.1:{port}")
                print("   ⚠️ 未检测到打印机，请检查打印机连接")
