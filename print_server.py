    ...
    try:
        ...
        try:
            ...
            if os.path.exists(edge_path):
                ...
                # Edge静默打印PDF
                cmd = f'"{edge_path}" --headless --disable-gpu --print-to-pdf=nul --run-all-compositor-stages-before-draw --disable-checker-imaging "{filepath}"'
                print(f"使用Edge打印: {cmd}")
                
                result = os.system(cmd)
                if result == 0:
                    print("✅ Edge PDF打印命令执行成功")
                    return True
        except Exception as e:
            print(f"❌ Edge打印失败: {e}")
...
def print_pdf_silent(filepath, printer_name, copies=1):
    """专门用于PDF文件的静默打印，优先使用国产软件"""
    try:
        # 方案1: 优先使用WPS PDF阅读器静默打印
        print("🥇 尝试WPS PDF静默打印...")
        wps_paths = [
            r"C:\Program Files (x86)\Kingsoft\WPS Office\ksopdfreader.exe",
            r"C:\Program Files\Kingsoft\WPS Office\ksopdfreader.exe",
        ...
        for wps_path in wps_paths:
            if os.path.exists(wps_path):
                try:
                    for i in range(copies):
                        # WPS静默打印命令
                        cmd = [wps_path, '/p', filepath]
                        result = subprocess.run(cmd, 
                                              capture_output=True, 
                                              creationflags=subprocess.CREATE_NO_WINDOW,
                                              timeout=30)
                        time.sleep(2)
                    return True, f"WPS PDF静默打印已发送到 {printer_name} ({copies}份)"
                except subprocess.TimeoutExpired:
                    return False, "WPS打印超时"
                except Exception as e:
                    print(f"WPS打印失败: {e}")
                    break
        
        # 方案2: 尝试Microsoft Edge PDF打印
        print("🥈 尝试Microsoft Edge PDF打印...")
        edge_path = r"C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe"
        if not os.path.exists(edge_path):
            edge_path = r"C:\Program Files\Microsoft\Edge\Application\msedge.exe"
        ...
        if os.path.exists(edge_path):
            try:
                # Edge可以打开PDF并使用系统打印对话框
                result = win32api.ShellExecute(0, 'printto', filepath, f'"{printer_name}"', None, 0)
                if result > 32:
                    return True, f"Edge PDF静默打印已发送到 {printer_name} ({copies}份)"
            except Exception as e:
                print(f"Edge打印失败: {e}")
        
        # 方案3: 使用Adobe Reader（备用方案）
        print("🥉 备用方案：尝试Adobe Reader...")
        adobe_paths = [
            r"C:\Program Files\Adobe\Acrobat DC\Acrobat\Acrobat.exe",
            r"C:\Program Files (x86)\Adobe\Acrobat Reader DC\Reader\AcroRd32.exe",
...
def delete_file_api():
    ...
    try:
        ...
        try:
            client_ip = request.remote_addr or '未知IP'
            log_message = f"{datetime.now()} 客户端: {client_ip} 删除文件: {filename}"
            with open(LOG_FILE, 'a', encoding='utf-8') as f:
                f.write(log_message + "\n")
...
def delete_all_files_api():
    ...
    try:
        ...
        # 记录删除日志
        client_ip = request.remote_addr or '未知IP'
        log_message = f"{datetime.now()} 客户端: {client_ip} 清空队列: 删除了 {deleted_count} 个文件"
        with open(LOG_FILE, 'a', encoding='utf-8') as f:
            f.write(log_message + "\n")
                ...
                if f and allowed_file(f.filename):
                    ...
                    try:
                        ...
                        if success:
                            ...
                            log_print(os.path.basename(filepath), printer, copies, duplex, papersize, quality, client_info)
                        else:
                            flash(f"❌ 打印失败: {message}", "danger")
                            log_print(os.path.basename(filepath) + f" 失败: {message}", printer, copies, duplex, papersize, quality, client_info)
                            
                    except Exception as e:
                        error_msg = f"打印异常: {str(e)}"
                        log_print(os.path.basename(filepath) + " " + error_msg, printer, copies, duplex, papersize, quality, client_info)
