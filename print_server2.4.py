...
def start_scan_silent(scanner_id, scanner_name, scan_format='PNG', scan_path=None):
    ...
    try:
        ...
        if scan_path is None:
            # 创建扫描文件专用目录
            scan_path = os.path.join(path_manager.get_app_dir(), 'scanned_files')
        ...
        # 直接使用Windows自带的扫描工具 - WiaAcmgr.exe
        print("正在打开扫描窗口，请完成扫描操作...")
        ...
        try:
            ...
            def launch_scanner():
                try:
                    ...
                    # 回退：用cmd启动
                    try:
                        subprocess.Popen(['cmd', '/c', 'WiaAcmgr.exe'], 
                                       creationflags=subprocess.CREATE_NO_WINDOW)
                    except Exception as e:
                        print(f"启动扫描窗口失败: {e}")
            
            scanner_thread = threading.Thread(target=launch_scanner, daemon=True)
            scanner_thread.start()
            
            # 等待一段时间（30秒），检查是否有新文件产生
            print("等待扫描完成（最多30秒）...")
            start_wait = time.time()
            max_wait = 30
            new_files = initial_files
            ...
            while time.time() - start_wait < max_wait:
                ...
                try:
                    ...
                    # 检查是否有新的扫描文件产生（过滤掉临时文件）
                    valid_new_files = [f for f in new_files if not f.startswith('~')]
                    if valid_new_files:
                        # 找到新文件
            ...
            return True, (
                "扫描窗口已打开。请注意：\n\n"
                "1. 完成扫描操作\n"
                "2. 扫描文件将自动保存到 scanned_files 文件夹\n"
                "3. 关闭扫描窗口即可"
            )
        
        except Exception as e:
            print(f"扫描启动异常: {e}")
            return False, f"无法启动扫描窗口: {str(e)}"
...
def api_trigger_scan():
    ...
    try:
        ...
        # 验证扫描格式
        allowed_formats = ('PNG', 'JPG', 'JPEG', 'PDF', 'BMP', 'TIFF')
...
<body>
<div class="container">
    <h1 class="mb-4 text-center">内网打印及扫描服务v2.4</br>
    <span class="author-info">（yckj666@52PJ，作者：忆痕）
    </p><a href="https://github.com/a937750307/lan-printing" target="_blank" style="color: #007bff; text-decoration: none;">【Github】</a>
    <a href="https://zanzhu.937788.xyz" target="_blank" style="color: #28a745; text-decoration: none;">【打赏】</a>
...
def print_image_silent(filepath, printer_name, copies=1):
    ...
    try:
        for i in range(copies):
            # 使用 ShellExecute 的 'print' 动词，打印到指定打印机
            try:
                ...
                if result <= 32:
                    ...
                    if result <= 32:
                        print(f"图片打印第{i+1}份失败，错误代码: {result}")
                time.sleep(2)
            except Exception as e:
                print(f"图片打印异常: {e}")
