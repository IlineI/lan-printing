...
def start_scan_silent(scanner_id, scanner_name, scan_format='PNG', scan_path=None):
    ...
    try:
        ...
        # 方法1: 使用comtypes.WIA进行静默扫描
        try:
            import comtypes.client as _comtypes_client
            print("尝试使用 comtypes.WIA 进行静默扫描...")
            result = _try_wia_scan_with_comtypes(scanner_id, full_scan_path, scan_format, scan_filename)
            if result:
                return result
        except Exception as e_comtypes:
            print(f"comtypes.WIA 扫描失败: {e_comtypes}")
        
        # 方法2: 使用win32com.WIA进行静默扫描
        try:
            import win32com.client
            print("回退：尝试使用 win32com.WIA 进行扫描...")
            result = _try_wia_scan_with_win32com(scanner_id, full_scan_path, scan_format, scan_filename)
            if result:
                return result
        except Exception as e_win32com:
            print(f"win32com.WIA 扫描失败: {e_win32com}")
        
        # 方法3: 最后回退 - 触发系统扫描UI
        try:
            print('最后回退：使用PowerShell触发系统扫描UI...')
            ps_script = "Start-Process -FilePath 'WiaAcmgr.exe' -ArgumentList '/SelectDevice' -WindowStyle Hidden"
            ...
            if result.returncode == 0:
                return False, '已触发系统扫描界面（需要手动确认），请完成扫描后检查 scanned_files 文件夹'
        except Exception as e_ps:
            print(f'PowerShell触发扫描UI失败: {e_ps}')
...
def _try_wia_scan_with_comtypes(scanner_id, full_scan_path, scan_format, scan_filename):
    """使用comtypes进行WIA扫描的辅助函数"""
    import comtypes.client as _comtypes_client
    import tempfile
    import subprocess
    ...
    try:
        ...
        if device is None and device_infos.Count > 0:
...
try:
    ...
    if sid and sid != 'default':
        for i in range(devs.Count):
            try:
                di = devs.Item(i+1)
                try:
                    if di.Properties('DeviceID').Value == sid:
                        dev = di.Connect()
                        break
                except:
                    pass
    ...
    if dev is None and devs.Count > 0:
...
"""
        ...
        try:
            ...
            if proc.returncode == 0 and 'OK' in output:
                if os.path.exists(full_scan_path) and os.path.getsize(full_scan_path) > 0:
                    size = os.path.getsize(full_scan_path)
                    return True, f"扫描成功，文件: {scan_filename} ({size} 字节)"
                else:
                    return False, '扫描文件为空'
            else:
        ...
        print(f"comtypes扫描异常: {e}")
        return None


def _try_wia_scan_with_win32com(scanner_id, full_scan_path, scan_format, scan_filename):
    """使用win32com进行WIA扫描的辅助函数"""
    import win32com.client
    import tempfile
    import subprocess
...
try:
    ...
    dm = wc.Dispatch('WIA.DeviceManager')
    dev = None
    devs = dm.DeviceInfos
    sid = sys.argv[1] if len(sys.argv) > 1 else ''
    out = sys.argv[2]
...
def print_office_silent(filepath, printer_name, copies=1):
    ...
    try:
        ...
        if file_ext in ['.doc', '.docx']:
            # Word 应用程序路径（包含多个版本和版次）
            office_paths = [
                # Office 2021 和 365（最新版本）
                ...
                # 家庭版、学生版等特殊版本
                r"C:\Program Files\Microsoft Office\Office17\WINWORD.EXE",
                r"C:\Program Files (x86)\Microsoft Office\Office17\WINWORD.EXE",
                
                # OneDrive/云版本特殊路径
                r"C:\Program Files\Microsoft OneDrive\WINWORD.EXE",
                r"C:\Program Files (x86)\Microsoft OneDrive\WINWORD.EXE",
                
                # 政府版、企业版等特殊渠道安装位置
                r"C:\Program Files\Microsoft\Office\WINWORD.EXE",
                r"C:\Program Files (x86)\Microsoft\Office\WINWORD.EXE",
                
                # 教育版
                r"C:\Program Files\Microsoft Office Education\WINWORD.EXE",
                r"C:\Program Files (x86)\Microsoft Office Education\WINWORD.EXE",
                
                # WPS Office 备选方案（如果Word未安装）
                r"C:\Program Files\kingsoft\WPS Office\office6\wpscloudsvr.exe",
                r"C:\Program Files (x86)\kingsoft\WPS Office\office6\wpscloudsvr.exe",
                r"C:\Program Files\WPS Office\office6\wpscloudsvr.exe",
                r"C:\Program Files (x86)\WPS Office\office6\wpscloudsvr.exe",
            ]
            app_name = "Word"
        elif file_ext in ['.xls', '.xlsx']:
            # Excel 应用程序路径（包含多个版本和版次）
            office_paths = [
                # Office 2021 和 365（最新版本）
                ...
                # 家庭版、学生版等特殊版本
                r"C:\Program Files\Microsoft Office\Office17\EXCEL.EXE",
                r"C:\Program Files (x86)\Microsoft Office\Office17\EXCEL.EXE",
                
                # OneDrive/云版本特殊路径
                r"C:\Program Files\Microsoft OneDrive\EXCEL.EXE",
                r"C:\Program Files (x86)\Microsoft OneDrive\EXCEL.EXE",
                
                # 政府版、企业版等特殊渠道安装位置
                r"C:\Program Files\Microsoft\Office\EXCEL.EXE",
                r"C:\Program Files (x86)\Microsoft\Office\EXCEL.EXE",
                
                # 教育版
                r"C:\Program Files\Microsoft Office Education\EXCEL.EXE",
                r"C:\Program Files (x86)\Microsoft Office Education\EXCEL.EXE",
                
                # WPS Office 备选方案（如果Excel未安装）
                r"C:\Program Files\kingsoft\WPS Office\office6\wpscsvr.exe",
                r"C:\Program Files (x86)\kingsoft\WPS Office\office6\wpscsvr.exe",
                r"C:\Program Files\WPS Office\office6\wpscsvr.exe",
                r"C:\Program Files (x86)\WPS Office\office6\wpscsvr.exe",
            ]
            app_name = "Excel"
        elif file_ext in ['.ppt', '.pptx']:
            # PowerPoint 应用程序路径（包含多个版本和版次）
            office_paths = [
                # Office 2021 和 365（最新版本）
                ...
                # 家庭版、学生版等特殊版本
                r"C:\Program Files\Microsoft Office\Office17\POWERPNT.EXE",
                r"C:\Program Files (x86)\Microsoft Office\Office17\POWERPNT.EXE",
                
                # OneDrive/云版本特殊路径
                r"C:\Program Files\Microsoft OneDrive\POWERPNT.EXE",
                r"C:\Program Files (x86)\Microsoft OneDrive\POWERPNT.EXE",
                
                # 政府版、企业版等特殊渠道安装位置
                r"C:\Program Files\Microsoft\Office\POWERPNT.EXE",
                r"C:\Program Files (x86)\Microsoft\Office\POWERPNT.EXE",
                
                # 教育版
                r"C:\Program Files\Microsoft Office Education\POWERPNT.EXE",
                r"C:\Program Files (x86)\Microsoft Office Education\POWERPNT.EXE",
                
                # WPS Office 备选方案（如果PowerPoint未安装）
                r"C:\Program Files\kingsoft\WPS Office\office6\wpppptsvr.exe",
                r"C:\Program Files (x86)\kingsoft\WPS Office\office6\wpppptsvr.exe",
                r"C:\Program Files\WPS Office\office6\wpppptsvr.exe",
                r"C:\Program Files (x86)\WPS Office\office6\wpppptsvr.exe",
            ]
            app_name = "PowerPoint"
        else:
            return print_file_silent_fallback(filepath, printer_name, copies)
