...
- 【支持多种文件格式打印】：JPG/PNG、PDF、PPT/PPTX、DOC/DOCX、XLS/XLSX和TXT等（建议转换成JPG和PNG打印）
...
- 扫描仪被锁定时，可点击"释放设备"按钮强制释放扫描仪，无需重启目标电脑或手动清理。
...
- 扫描仪被锁定/提示"设备忙"：这是由于前次扫描未正确释放 COM 对象导致，可以：
  - 方案1：点击"释放设备"按钮，程序会自动尝试多层级释放策略（WIA服务重启、进程清理、内存回收），无需重启电脑
  - 方案2：刷新页面后重试扫描，程序扫描前会自动检测并释放被锁定的设备
  - 方案3：如问题仍未解决，可进行以下手动操作：
    - Windows 10/11 用户：打开设备管理器，找到扫描仪，卸载驱动后重新安装
    - Windows 7 用户：直接使用对应的修复工具（win7_fix_tool_2.3.py）进行修复
- 扫描文件占用磁盘空间：扫描文件会在30分钟后自动删除，如需立即清空，可点击"清空队列"按钮手动删除所有扫描文件
