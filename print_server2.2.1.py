...
try:
    ...
    try:
        ...
        # 回退：用 TextIOWrapper 包装 stdout/stderr
        try:
...
# 设置 PYTHONIOENCODING 环境变量以避免 subprocess 或子进程的输出编码问题
try:
    os.environ.setdefault('PYTHONIOENCODING', 'utf-8')
except Exception:
    pass

# 尝试设置 locale 到中文 UTF-8，帮助在某些控制台中正确显示中文
try:
    import locale
    try:
...
# 额外保护：确保 stdout/stderr 的 encoding 属性为 utf-8（某些嵌入式控制台需要）
try:
...
def print_console_diagnostics():
    """在启动时打印控制台与编码相关的诊断信息，便于排查中文/编码问题"""
    try:
        lines = []
        stdout_enc = getattr(sys.stdout, 'encoding', None)
        stderr_enc = getattr(sys.stderr, 'encoding', None)
        lines.append(('标准输出编码', stdout_enc))
        lines.append(('标准错误编码', stderr_enc))
        try:
            import locale
            lines.append(('locale.getdefaultlocale', locale.getdefaultlocale()))
            lines.append(('首选编码 (locale.getpreferredencoding)', locale.getpreferredencoding()))
        except Exception:
            lines.append(('locale.getdefaultlocale', None))
            lines.append(('首选编码 (locale.getpreferredencoding)', None))

        lines.append(('环境变量 PYTHONIOENCODING', os.environ.get('PYTHONIOENCODING')))

        # Windows 控制台 code page
        if os.name == 'nt':
            try:
                import ctypes
                cp_in = ctypes.windll.kernel32.GetConsoleCP()
                cp_out = ctypes.windll.kernel32.GetConsoleOutputCP()
            except Exception:
                cp_in = None
                cp_out = None
            lines.append(('控制台输入代码页 (GetConsoleCP)', cp_in))
            lines.append(('控制台输出代码页 (GetConsoleOutputCP)', cp_out))
...
# 立即打印诊断信息，便于在服务启动日志中查看
try:
    print_console_diagnostics()
except Exception:
    pass
# 注意：WIA 自动化依赖 comtypes 库的部分子模块（例如 comtypes.stream）。
# 如果在运行时遇到类似 "No module named 'comtypes.stream'" 错误，建议在目标机器上手动执行：
#
#   pip install comtypes
#   pip install pywin32
#
# 本脚本在检测到 comtypes 不可用或失败时，会回退到 win32com 或 PowerShell 的方法进行扫描/枚举，
# 但完全静默扫描可能无法在所有驱动或设备上工作。若需要更可靠的WIA自动化，请手动安装上述依赖。
...
class ServiceManager:
    ...
    def _optimize_for_windows_version(self):
        ...
        try:
            ...
            if is_win7:
                # Win7保守设置：更频繁检查，更少重启
                self.health_check_interval = 300  # 5分钟检查
                self.max_restart_attempts = 3  # 最多重启3次
                self.restart_cooldown = 600  # 10分钟冷却
                print(" Win7服务管理：保守策略")
                
            elif is_win11:
                # Win11激进设置：检查间隔长，重启次数多
                self.health_check_interval = 900  # 15分钟检查
                self.max_restart_attempts = 8  # 最多重启8次
                self.restart_cooldown = 180  # 3分钟冷却
                print(" Win11服务管理：高性能策略")
                
            else:
                # Win10平衡设置
                self.max_restart_attempts = 5  # 最多重启5次
                self.restart_cooldown = 300  # 5分钟冷却
                print(" Win10服务管理：平衡策略")
            ...
            self.restart_count = 0
            self.last_restart_time = 0
            print(f"Windows版本检测失败，使用默认服务管理设置: {e}")
...
def get_available_scanners():
    ...
    # 方法2: 通过注册表检查已安装的扫描仪
        try:
            ...
            # 扫描注册表中的扫描仪信息
            reg_paths = [
                (winreg.HKEY_LOCAL_MACHINE, r"SYSTEM\CurrentControlSet\Control\Class\{6bdd1fc6-810f-11d0-bec7-08002be2092f}"),
                (winreg.HKEY_LOCAL_MACHINE, r"SOFTWARE\Microsoft\Windows\CurrentVersion\StillImage\Registered Applications"),
            ]
                            ...
                            try:
                                ...
                                with winreg.OpenKey(key, subkey_name) as subkey:
                                    try:
                                        ...
                                        if "scan" in device_desc.lower() or "扫描" in device_desc:
                                            # 避免重复添加
                                            exists = any(s['name'] == device_desc for s in scanners)
                                            if not exists:
                                                scanners.append({
                                                    'name': device_desc,
                                                    'id': f"REG_{i}",
...
'''
    ...
    # 方法4: 基于已安装打印机推断多功能设备（MFP）——许多用户的打印机同时支持扫描
        try:
            ...
            # 常见启发式关键词（包括英文与中文）
            heuristics = ['scan', '扫描', 'mfp', 'multi', 'all-in-one', 'all in one', '多功能', 'fax', '传真']
            vendor_kw = ['hp', 'canon', 'brother', 'epson', 'xerox', 'ricoh', 'konica', 'kyocera']

            # 只检查有限数量的打印机以防止阻塞
            for p in list(printers)[:50]:
                try:
                    pname = str(p)
                    lname = pname.lower()

                    # 如果名称明显包含扫描相关词，直接添加候选
                    if any(k in lname for k in heuristics + vendor_kw):
                        if not any(s['name'] == pname for s in scanners):
                            scanners.append({'name': pname, 'id': f'PRINTER_{pname}', 'type': 'PrinterCandidate', 'available': True})
                            print(f"基于打印机推断为多功能扫描设备: {pname}")
                            continue

                    # 否则尝试通过PnP匹配：查找与打印机名称相关联的 Image 类设备
                    try:
                        safe_name = pname.replace("'", "''")
                        ps = f"Get-PnpDevice | Where-Object {{ $_.FriendlyName -like '*{safe_name}*' -and ($_.Class -eq 'Image') }} | Select-Object FriendlyName, InstanceId, Status | ConvertTo-Json"
                        result = subprocess.run(['powershell', '-Command', ps], capture_output=True, text=True, timeout=6, creationflags=subprocess.CREATE_NO_WINDOW)
                        if result.returncode == 0 and result.stdout.strip() and result.stdout.strip() != '[]':
                            try:
                                import json as _json
                                devs = _json.loads(result.stdout)
                                if not isinstance(devs, list):
                                    devs = [devs]
                                for d in devs:
                                    dname = d.get('FriendlyName') or pname
                                    iid = d.get('InstanceId') or f'PNP_{pname}'
                                    if not any(s['name'] == dname for s in scanners):
                                        scanners.append({'name': dname, 'id': iid, 'type': 'PnP-Mapped', 'available': d.get('Status', '') in ('OK', 'Started', 'OK (Started)')})
                                        print(f"通过PnP匹配到与打印机关联的扫描设备: {dname} (来自打印机: {pname})")
                            except Exception as e_jsonmap:
                                print(f"解析PnP匹配结果失败: {e_jsonmap}")
                    except Exception as e_psmap:
                        # 忽略单个打印机的匹配错误
                        # print(f"PnP匹配失败 for {pname}: {e_psmap}")
                        pass
    ...
    # 方法5: 使用网络探针进一步发现（基于打印机 IP）并合并
        try:
            ...
            for nm in net_mfps:
                # 把网络发现作为候选项，避免重复
                nm_name = nm.get('printer_name') or nm.get('ip')
                if not any(s['name'] == nm_name for s in scanners):
                    scanners.append({'name': nm_name, 'id': f"NET_{nm.get('ip')}", 'type': 'NetworkMFP', 'available': True, 'vendor': nm.get('vendor'), 'snmp': nm.get('snmp')})
        except Exception as e:
            print(f"合并网络MFP发现失败: {e}")
        ...
        if not scanners:
            ...
            print("未检测到具体扫描仪，已添加默认选项")

        # 给所有扫描仪添加 vendor/confidence 标注（本地和网络发现均会标注）
        try:
            _annotate_scanners_with_vendor(scanners)
        except Exception:
            pass
...
def start_scan_silent(scanner_id, scanner_name, scan_format='PNG', scan_path=None):
    """静默扫描功能 - 无弹窗自动确认"""
    try:
        print(f"开始静默扫描: {scanner_name} ({scanner_id})")
        
        if scan_path is None:
            # 创建扫描文件专用目录
            scan_path = os.path.join(os.path.dirname(__file__), 'scanned_files')
        ...
        scan_filename = f"scan_{timestamp}.{scan_format.lower()}"
        full_scan_path = os.path.join(scan_path, scan_filename)
        
        # 方法1: 使用WIA COM对象进行静默扫描（优先）
        tried_wia = False
        try:
            tried_wia = True
            try:
                import comtypes.client as _comtypes_client
                print("尝试使用 comtypes.WIA 进行静默扫描...")
                device_manager = _comtypes_client.CreateObject("WIA.DeviceManager")

                # 查找或连接设备
                device = None
                device_infos = device_manager.DeviceInfos
                if scanner_id and scanner_id != 'default':
                    for i in range(device_infos.Count):
                        di = device_infos.Item(i + 1)
                        try:
                            if di.Properties('DeviceID').Value == scanner_id:
                                device = di.Connect()
                                break
                        except Exception:
                            continue
                ...
                if device is None:
                    if device_infos.Count > 0:
                        device = device_infos.Item(1).Connect()
                    else:
                        raise RuntimeError('未找到可用的WIA扫描仪')

                # 获取第一个可用项
                items = device.Items
                if getattr(items, 'Count', 0) == 0:
                    raise RuntimeError('扫描仪无可用扫描项目')

                scanner_item = items.Item(1)

                # 选择格式GUID
                fmt = scan_format.upper() if scan_format else 'PNG'
                if fmt == 'PDF':
                    format_guid = '{B96B3CAE-0728-11D3-9D7B-0000F81EF32E}'
                elif fmt in ('JPG', 'JPEG'):
                    format_guid = '{B96B3CAF-0728-11D3-9D7B-0000F81EF32E}'
                else:
                    format_guid = '{B96B3CAF-0728-11D3-9D7B-0000F81EF32E}'

                # 保护性设置参数，部分设备对属性索引不一致
                try:
                    # 分辨率属性常用ID为 6147 或 6146，根据设备容错
                    for prop_id in ('6147', '6146'):
                        try:
                            scanner_item.Properties(prop_id).Value = 300
                        except Exception:
                            continue

                    # 颜色模式保护性设置
                    for prop_id in ('6146', '6147'):
                        try:
                            scanner_item.Properties(prop_id).Value = 4
                        except Exception:
                            continue
                except Exception:
                    pass

                # 执行传输并保存 —— 在单独子进程中运行以避免阻塞主进程或弹出长期的设备UI
                print('执行WIA扫描传输（子进程）...')

                try:
                    # 构建子进程脚本（尽量自包含，避免外部依赖）
                    child_script = r"""
import sys
try:
    import comtypes.client as _comtypes_client
    dm = _comtypes_client.CreateObject('WIA.DeviceManager')
    dev = None
    devs = dm.DeviceInfos
    scanner_id_arg = sys.argv[1] if len(sys.argv) > 1 else ''
    out_path = sys.argv[2]
    fmt = sys.argv[3]

    # 选择设备
    if scanner_id_arg and scanner_id_arg != 'default':
        for i in range(devs.Count):
            try:
                di = devs.Item(i+1)
                try:
                    if di.Properties('DeviceID').Value == scanner_id_arg:
                        dev = di.Connect()
                        break
                except Exception:
                    continue
    ...
    print('ERR_CHILD:'+str(e))
    sys.exit(6)
"""

                    # 调用子进程执行扫描（短超时，避免弹窗长期阻塞）
                    import tempfile, subprocess
                    python_exe = sys.executable or 'python'
                    # 写临时脚本到文件，避免复杂的命令行引号问题
                    with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False, encoding='utf-8') as tf:
                        tf.write(child_script)
                        script_path = tf.name

                    try:
                        cmd = [python_exe, script_path, str(scanner_id or ''), full_scan_path, format_guid]
                        # 设置较短超时，若设备会弹出UI导致阻塞，则超时并杀死子进程，避免用户必须手动确认
                        proc = subprocess.run(cmd, capture_output=True, text=True, timeout=20)
                        stdout = (proc.stdout or '') + (proc.stderr or '')
                        if proc.returncode == 0 and 'OK' in stdout:
                            if os.path.exists(full_scan_path) and os.path.getsize(full_scan_path) > 0:
                                file_size = os.path.getsize(full_scan_path)
                                return True, f"扫描成功，文件: {scan_filename} ({file_size} 字节)"
                            else:
                                return False, '扫描完成但保存的文件为空'
                        else:
                            print('子进程扫描失败，输出:', stdout.strip())
                            # 如果子进程输出提示需要UI或其他错误，返回明确提示，不再触发系统扫描UI
                            return False, '此扫描设备不支持完全静默扫描，已中止以避免弹出交互窗口。请在目标机器上手动完成扫描，或在设置中允许交互式扫描。'
                    except subprocess.TimeoutExpired:
                        try:
                            proc.kill()
                        except Exception:
                            pass
                        return False, '扫描超时：设备可能会弹出交互窗口，当前以静默模式中止。'
                    finally:
                        try:
                    ...
                    print(f'子进程扫描执行异常: {e_child}')
                    return False, '执行子进程扫描时发生错误'

            except Exception as e_wia:
                print(f"WIA(comtypes) 扫描失败: {e_wia}")
                # 若 comtypes 缺少内部模块（如 comtypes.stream），不要尝试自动安装；回退到 win32com 或 PowerShell
        except Exception as e:
            print(f"WIA扫描逻辑异常: {e}")
            
            # 方法2: 回退到 win32com (Dispatch WIA) 或 PowerShell 主动触发用户扫描流程
            try:
                # 尝试 win32com 回退
                try:
                    import win32com.client as win32com_client
                    print('回退：尝试使用 win32com Dispatch(WIA.DeviceManager) 进行扫描')
                    dm = win32com_client.Dispatch('WIA.DeviceManager')
                    device = None
                    devs = dm.DeviceInfos
                    if scanner_id and scanner_id != 'default':
                        for i in range(1, devs.Count + 1):
                            try:
                                di = devs.Item(i)
                                if getattr(di.Properties, 'Item', lambda x: None)('DeviceID') and di.Properties('DeviceID').Value == scanner_id:
                                    device = di.Connect()
                                    break
                            except Exception:
                                continue

                    if device is None and devs.Count > 0:
                        device = devs.Item(1).Connect()

                    if device is not None:
                        items = device.Items
                        if getattr(items, 'Count', 0) == 0:
                            raise RuntimeError('设备无可用扫描项')
                        scanner_item = items.Item(1)
                        # 使用与comtypes类似的保存流程
                        try:
                            fmt = scan_format.upper() if scan_format else 'PNG'
                            ...
                            # 与 comtypes 相同，使用子进程调用 win32com 进行 Transfer，避免UI阻塞
                            child_script = r"""
import sys
try:
    import win32com.client as _win32
    dm = _win32.Dispatch('WIA.DeviceManager')
    dev = None
    devs = dm.DeviceInfos
    scanner_id_arg = sys.argv[1] if len(sys.argv) > 1 else ''
    out_path = sys.argv[2]
...
"""
                            ...
                            try:
                                ...
                                if proc.returncode == 0 and 'OK' in stdout:
                                    if os.path.exists(full_scan_path) and os.path.getsize(full_scan_path) > 0:
                                        return True, f"扫描成功，文件: {scan_filename}"
                                    else:
                                        return False, '扫描完成但保存的文件为空'
                                else:
                                    print('子进程(win32com)扫描失败，输出:', stdout.strip())
                                    return False, '此扫描设备不支持完全静默扫描，已中止以避免弹出交互窗口。'
                            except subprocess.TimeoutExpired:
                                try:
                                    proc.kill()
                                except Exception:
                            ...
                            print(f'win32com 扫描/保存失败: {e_img}')

                except Exception as e_win32com:
                    print(f'win32com 回退不可用或失败: {e_win32com}')

                # 最后回退：通过PowerShell触发系统扫描UI（非完全静默，但可作为回退）
                print('回退：使用PowerShell触发系统扫描UI（需要用户确认）')
                ps_script = f"Start-Process -FilePath 'WiaAcmgr.exe' -ArgumentList '/SelectDevice' -WindowStyle Hidden"
                result = subprocess.run(['powershell', '-Command', ps_script], capture_output=True, text=True, timeout=20, creationflags=subprocess.CREATE_NO_WINDOW)
                if result.returncode == 0:
                    return False, '已触发系统扫描界面（非静默），请在目标机器上完成扫描并检查扫描结果文件夹'
                else:
                    print(f'PowerShell触发扫描界面返回码: {result.returncode}, stdout={result.stdout}, stderr={result.stderr}')

            except Exception as e_final:
                print(f'回退扫描方法全部失败: {e_final}')
            
            # 所有静默扫描方法都失败了，返回详细诊断建议
            diagnostics = [
                '未检测到可用的静默扫描路径',
                '可能原因：',
                '• 扫描仪未正确连接或未开机',
                '• 缺少或损坏的WIA驱动程序',
                "• 本机缺少 'comtypes' 或 win32com 模块，导致WIA自动化失败",
                '• 扫描仪或驱动不支持无UI静默扫描',
                '• 权限不足或安全软件阻止应用访问扫描设备',
            ]
            return False, "\n".join(diagnostics)
...
# ----------------- 网络多功能打印机（MFP）发现与拉取扫描（厂商探针） -----------------
def _extract_ip_from_text(text):
    """从文本中提取第一个IPv4地址（如果有）。"""
    import re
    if not text:
        return None
    m = re.search(r'(?:25[0-5]|2[0-4]\d|1?\d?\d)(?:\.(?:25[0-5]|2[0-4]\d|1?\d?\d)){3}', str(text))
    return m.group(0) if m else None


def probe_snmp_sysdescr(ip, timeout=2):
    """尝试通过 SNMP 获取 sysDescr（需要 pysnmp）。返回字符串或 None。"""
    try:
        # 使用 importlib 动态导入，避免静态分析报错
        import importlib
        try:
        ...
        iterator = getCmd(SnmpEngine(), CommunityData('public', mpModel=0), UdpTransportTarget((ip, 161), timeout=timeout, retries=0), ContextData(), ObjectType(ObjectIdentity('1.3.6.1.2.1.1.1.0')))
...
def probe_http_for_vendor(ip, timeout=2):
    """通过 HTTP 探针尝试识别厂商，返回 vendor 字符串或 None。
    尝试使用 requests（若可用），否则 urllib。
    """
    vendor = None
    # 常见路径（短列表，避免大量探测）
    paths = ['/', '/index.html', '/hp/device/this.LCDispatcher', '/hp/device/this.LCDispatcher?OS=1', '/web/guest/en/web/gui', '/wsd', '/xml/deviceinfo', '/Device/All.xml', '/sws/Generic', '/brother/asp/printerstatus.html']
    # 常见厂商指纹关键词映射
    vendor_map = {
        'hp': ['hp', 'hpjetdirect', 'hewlett-packard', 'hpe'],
        'canon': ['canon', 'imageclass', 'canon ir', 'canon mf'],
        'brother': ['brother', 'brweb', 'brftp'],
        'epson': ['epson', 'epsonweb', 'epson status'],
        'xerox': ['xerox', 'xerox Phaser', 'xerox WorkCentre'],
        'ricoh': ['ricoh', 'ricoh mp'],
    ...
    def _check_text_for_vendor(text):
        if not text:
            return None
        t = text.lower()
        for v, keys in vendor_map.items():
            for k in keys:
                if k in t:
                    return v.capitalize()
        return None
        ...
        for p in paths:
            try:
                ...
                try:
                    ...
                    joint = ' '.join([str(r.headers), r.text or ''])
                except Exception:
                    try:
                        from urllib.request import urlopen, Request
                        req = Request(url, headers={'User-Agent': 'Mozilla/5.0'})
                        with urlopen(req, timeout=timeout) as resp:
                            joint = ' '.join([str(resp.getheaders()), resp.read(10240).decode('utf-8', errors='ignore')])
                    except Exception:
                        continue

                vendor_detected = _check_text_for_vendor(joint)
                if vendor_detected:
                    return vendor_detected
...
def discover_network_mfps(timeout=1):
    """尝试在已知打印机列表上发现网络多功能设备（返回候选列表）。
    逻辑：从 `PRINTERS` 中提取 IP（若打印机名称中包含IP），对该IP进行 HTTP/SNMP 探针，并返回发现到的厂商信息。
    非侵入式、短超时。
    """
    results = []
    try:
        printers = globals().get('PRINTERS') or []
        seen_ips = set()
        for p in list(printers)[:100]:
            try:
                pname = str(p)
                ip = _extract_ip_from_text(pname)
                if not ip:
                    # 有些打印机名称可能包含 (IP:xxx) 或类似格式，尝试额外提取
                    ip = _extract_ip_from_text(pname.replace(':', '.'))
                if not ip:
                    continue
                if ip in seen_ips:
                    continue
                ...
                vendor = probe_http_for_vendor(ip, timeout=timeout)
                sn = None
                if not vendor:
                    sn = probe_snmp_sysdescr(ip, timeout=timeout)
                    if sn:
                        lv = str(sn).lower()
                        if 'hp' in lv:
                            vendor = 'HP'
                ...
                # 如果任一方法提示厂商，则添加候选
                if vendor or sn:
                    results.append({'ip': ip, 'printer_name': pname, 'vendor': vendor or 'Unknown', 'snmp': sn})
            except Exception:
                continue
    except Exception as e:
        print(f"网络MFP发现异常: {e}")
...
def _detect_vendor_from_text(text):
    """从设备名称或描述中启发式识别厂商，返回 (vendor, confidence) 或 (None, 0)。"""
    if not text:
        return None, 0
    t = str(text).lower()
    # 更全面的厂商关键字列表
    vendor_keywords = {
        'hp': ['hp', 'hewlett-packard', 'hpjetdirect', 'hp multifunction', 'hp officejet', 'hp laserjet'],
        'canon': ['canon', 'imageclass', 'imageRUNNER', 'mf', 'canon i-sensys'],
        'brother': ['brother', 'brweb', 'brother mfc', 'brother dcp'],
        'epson': ['epson', 'epogate', 'epson status', 'epson workforce'],
        'xerox': ['xerox', 'phaser', 'workcentre'],
        'ricoh': ['ricoh', 'mp c', 'mp c'],
        'konica': ['konica', 'konica minolta'],
        'kyocera': ['kyocera'],
        'lexmark': ['lexmark'],
        'sharp': ['sharp'],
    ...
    # exact matches give higher confidence
    for vendor, keys in vendor_keywords.items():
        for k in keys:
            if k.lower() in t:
                # confidence: longer match or exact word gives higher
                conf = 80 if len(k) > 4 else 60
                # boost if vendor name appears as a standalone word
                if (' ' + k + ' ') in (' ' + t + ' '):
                    conf = min(95, conf + 10)
                return vendor.capitalize(), conf

    return None, 0


def _annotate_scanners_with_vendor(scanners):
    """为 scanners 列表中的每一项添加 vendor 和 confidence 字段（如果尚未存在）。"""
    try:
        for s in scanners:
            if not isinstance(s, dict):
                continue
            if 'vendor' in s and s['vendor']:
                continue

            # 尝试从 name, id, type, snmp, or other fields中识别
            candidates = []
            if s.get('name'):
            ...
            if s.get('type'):
                candidates.append(s.get('type'))
            if s.get('snmp'):
                candidates.append(s.get('snmp'))

            best_vendor = None
            best_conf = 0
            for c in candidates:
                v, conf = _detect_vendor_from_text(c)
                if v and conf > best_conf:
                    best_vendor, best_conf = v, conf

            # 如果 network discovery 提供 vendor 优先使用
            if not best_vendor and s.get('vendor'):
                best_vendor = s.get('vendor')
                best_conf = s.get('confidence', 50)

            s['vendor'] = best_vendor or 'Unknown'
            s['confidence'] = int(best_conf)
    except Exception as e:
        print(f"标注扫描仪厂商时出错: {e}")
...
# Flask配置优化，针对不同Windows版本
def get_optimized_flask_config():
    """根据Windows版本返回优化的Flask配置"""
    import platform
    import sys
    ...
    try:
        ...
        if is_win7:
            # Win7兼容性配置
            base_config.update({
                'SEND_FILE_MAX_AGE_DEFAULT': 60,  # 减少缓存时间
                'MAX_CONTENT_LENGTH': 50 * 1024 * 1024,  # 降低文件大小限制
                'PERMANENT_SESSION_LIFETIME': 1800,  # 30分钟会话
            })
            print("应用Win7兼容性Flask配置")
            
        elif is_win11:
            # Win11高性能配置
            base_config.update({
                'SEND_FILE_MAX_AGE_DEFAULT': 600,  # 增加缓存时间
                'MAX_CONTENT_LENGTH': 200 * 1024 * 1024,  # 更大文件支持
                'PERMANENT_SESSION_LIFETIME': 7200,  # 2小时会话
                'PREFERRED_URL_SCHEME': 'http',
                'APPLICATION_ROOT': '/',
            })
            print("应用Win11高性能Flask配置")
            
        else:
            # Win10标准配置
            print("应用Win10标准Flask配置")
    
    except Exception as e:
        print(f"Flask配置检测失败，使用默认配置: {e}")
    
    return base_config

app.config.update(get_optimized_flask_config())# 兼容PyInstaller打包的路径处理
# 使用路径管理器配置文件夹和文件路径
...
# POST JSON: { "scanner_id": "...", "scanner_name": "...", "format": "PNG" }
...
def api_trigger_scan():
    try:
        ...
        try:
            ...
            resp = jsonify({'success': ok, 'message': message})
            resp.headers['Access-Control-Allow-Origin'] = '*'
        ...
        <div class="main-content">
                ...
                <div class="card-body">
                    <form id="scanForm" class="row g-3">
                        <div class="col-md-6">
                            <label class="form-label">选择扫描仪 
                                <button type="button" class="btn btn-sm btn-outline-secondary ms-2" onclick="refreshScannerList()" title="刷新扫描仪列表">
                                     刷新
                                </button>
                            </label>
                    ...
                    <div class="alert alert-info mt-3">
                        ...
                        <ul class="mb-0 small">
                            ...
                            <li><strong>️支持格式:</strong> PNG、JPEG、PDF格式，建议使用PNG获得最佳质量</li>
                            <li><strong>️自动设置:</strong> 尝试自动设置300 DPI分辨率和彩色扫描模式</li>
                            <li><strong>自动保存:</strong> 扫描完成的文件会保存在【扫描文件】区域，支持预览、下载和打印</li>
                            <li><strong>兼容性:</strong> 兼容大多数品牌的扫描仪，如Canon、HP、Epson等</li>
                            <li><strong>️失败处理:</strong> 如果确实安装了扫描仪但扫描失败，请在目标电脑上进行扫描操作</li>
                        </ul>
                    </div>
                </div>
            </div>
            ...
            <div class="card mb-4" style="border-left: 4px solid #28a745;">
                <div class="card-header bg-light d-flex justify-content-between align-items-center">
                    ...
                    <button type="button" class="btn btn-sm btn-outline-success" onclick="refreshScannedFiles()" title="刷新扫描文件列表">
                         刷新
                    </button>
                </div>
                <div class="card-body">
...
function performScan() {
    ...
    formData.append('scanner_id', scannerSelect.value);
    formData.append('scanner_name', scannerSelect.options[scannerSelect.selectedIndex].text);
...
def get_scanned_files():
    ...
    try:
        # 确保扫描目录存在
        scan_folder = os.path.join(os.path.dirname(__file__), 'scanned_files')
...
def on_set_current_ip_static(icon, item):
    ...
    try:
        ...
        if current_ip == '127.0.0.1':
            messagebox.showerror("无效IP", "当前IP为本地回环地址，无法设置为静态IP")
            root.destroy()
            return
        ...
        if result:
            ...
            if success:
                ...
                messagebox.showinfo("设置成功", f"已将当前IP设置为静态IP\n\n静态IP: {current_ip}\n\n注意：托盘菜单中的IP地址可能需要几秒钟才会更新")
                ...
                messagebox.showerror("设置失败", f"设置失败: {message}")
        
        root.destroy()
        
    except Exception as e:
        try:
            messagebox.showerror("错误", f"设置静态IP时发生错误: {str(e)}")
            root.destroy()
        except:
            print(f"设置静态IP失败: {e}")
...
def on_enable_dhcp(icon, item):
    ...
    try:
        ...
        if success:
            messagebox.showinfo("DHCP设置成功", f"{message}\n\n注意：新IP地址可能需要几秒钟才会在托盘菜单中显示")
...
def build_menu(icon):
    ...
    return pystray.Menu(
        ...
        pystray.MenuItem('打赏作者', on_donate),
        pystray.MenuItem('安装二维码生成插件', on_open_qr_plugin),
    ...
    try:
        ...
        if logo_path:
            try:
                ...
                # Win11高DPI支持
                if is_win11 and hasattr(image, 'size') and max(image.size) < 64:
                    # Win11建议使用更高分辨率图标
                    image = image.resize((64, 64), Image.Resampling.LANCZOS)
                    print(f"Win11图标已优化为64x64: {image.size}")
                elif is_win7:
                    # Win7使用较小图标避免兼容性问题
                    if hasattr(image, 'size') and max(image.size) > 32:
                        image = image.resize((32, 32), Image.Resampling.LANCZOS)
                        print(f"Win7图标已优化为32x32: {image.size}")
                else:
        ...
        if not logo_path:
            ...
            # 根据Windows版本创建不同尺寸的默认图标
            if is_win11:
                image = Image.new('RGB', (64, 64), color='blue')
                draw = ImageDraw.Draw(image)
                draw.text((24, 24), "P", fill='white')
            else:
        ...
        if is_win7:
            print("Win7系统托盘启动（兼容模式）")
        elif is_win11:
            print("Win11系统托盘启动（高性能模式）")
        else:
            print("Win10系统托盘启动（标准模式）")
...
if __name__ == '__main__':
    try:
        ...
        if hasattr(sys, '_MEIPASS'):
            ...
            is_win7 = windows_version == "7"
            is_win10 = windows_version == "10"
            is_win11 = windows_build and windows_build >= 22000 if windows_build else False
            
            if is_win7:
                print("️ 检测到Windows 7系统，启用兼容模式...")
                print(" Win7模式：启动时提供 5 秒选择保留控制台（按任意键保留），否则自动隐藏以最小化干扰")
                try:
                    import msvcrt
                    # 显示提示并等待 5 秒，期间按任意键可保留控制台
                    sys.stdout.write("按任意键可在5秒内保留控制台窗口，否则程序将隐藏控制台并在后台运行...\r\n")
                    sys.stdout.flush()
                    start = time.time()
                    keep_console = False
                    ...
                    if keep_console:
                        ...
                        try:
                            ...
                            if CONSOLE_WINDOW:
                                ...
                                CONSOLE_VISIBLE = False
                                print("未检测到按键，控制台已隐藏；程序将在后台运行。")
                        except Exception as e:
                            print(f"隐藏控制台失败: {e}")
                except Exception as e:
                    print(f"Win7 控制台等待逻辑失败: {e}")
                
            elif is_win11:
                print("️ 检测到Windows 11系统，启用高级模式...")
                print(" Win11优化：增强安全性和性能")
                ...
                try:
                    ...
                    print(f"️ Win11控制台处理失败: {e}")
                    print(" 控制台将保持可见")
                    
            elif is_win10:
                print(" 检测到Windows 10系统，启用标准模式...")
                print(" Win10优化：平衡性能和兼容性")
                ...
                try:
                    ...
                    print(f"️ Win10控制台处理失败: {e}")
                    print(" 控制台将保持可见")
                    
            else:
                # 未知Windows版本，使用保守设置
                print(f"️ 检测到Windows {windows_version}系统，使用通用模式...")
                print(" 通用模式：保守的兼容性设置")
                # 保持控制台可见，确保兼容性
